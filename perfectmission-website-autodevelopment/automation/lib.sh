...
AUTODEV_LEARNED_SKILLS_DIR="${AUTODEV_LEARNED_SKILLS_DIR:-$AUTODEV_SKILLS_DIR/learned}"
...
AUTODEV_LEARNED_SKILLS_SUMMARY_FILE="${AUTODEV_LEARNED_SKILLS_SUMMARY_FILE:-$AUTODEV_STATE_DIR/learned-skills.md}"
AUTODEV_LEARNED_SKILLS_MAX="${AUTODEV_LEARNED_SKILLS_MAX:-12}"
...
AUTODEV_MEMORY_BACKEND="${AUTODEV_MEMORY_BACKEND:-hindsight-hybrid}"
...
AUTODEV_MEMORY_HINDSIGHT_API_URL="${AUTODEV_MEMORY_HINDSIGHT_API_URL:-${HINDSIGHT_API_URL:-http://127.0.0.1:9077}}"
AUTODEV_MEMORY_HINDSIGHT_API_TOKEN="${AUTODEV_MEMORY_HINDSIGHT_API_TOKEN:-${HINDSIGHT_API_TOKEN:-}}"
AUTODEV_MEMORY_HINDSIGHT_BANK_ID="${AUTODEV_MEMORY_HINDSIGHT_BANK_ID:-kraliki-autodev-$AUTODEV_ID}"
AUTODEV_MEMORY_HINDSIGHT_BANK_MISSION="${AUTODEV_MEMORY_HINDSIGHT_BANK_MISSION:-Track project objectives, architecture, invariants, commands, failures, delivery history, open questions, and next actions. Ignore boilerplate and low-signal chatter.}"
AUTODEV_MEMORY_HINDSIGHT_OBSERVATIONS_MISSION="${AUTODEV_MEMORY_HINDSIGHT_OBSERVATIONS_MISSION:-Observations are stable facts about the project, operator workflow, recurring failures, and delivery patterns. Ignore one-off transient task noise.}"
AUTODEV_MEMORY_HINDSIGHT_STATE_FILE="${AUTODEV_MEMORY_HINDSIGHT_STATE_FILE:-$AUTODEV_MEMORY_BACKEND_DIR/hindsight-sync.json}"
AUTODEV_MEMORY_HINDSIGHT_RECALL_BUDGET="${AUTODEV_MEMORY_HINDSIGHT_RECALL_BUDGET:-mid}"
AUTODEV_MEMORY_HINDSIGHT_MAX_TOKENS="${AUTODEV_MEMORY_HINDSIGHT_MAX_TOKENS:-1400}"
AUTODEV_MEMORY_HINDSIGHT_TIMEOUT_SECONDS="${AUTODEV_MEMORY_HINDSIGHT_TIMEOUT_SECONDS:-120}"
AUTODEV_MEMORY_HINDSIGHT_TAGS="${AUTODEV_MEMORY_HINDSIGHT_TAGS:-autodev,project:$PROJECT_NAME,autodev-id:$AUTODEV_ID}"
AUTODEV_MEMORY_HINDSIGHT_RECOVERY_OPENCLAW_CONFIG="${AUTODEV_MEMORY_HINDSIGHT_RECOVERY_OPENCLAW_CONFIG:-$HOME/.openclaw/openclaw.json}"
AUTODEV_MEMORY_HINDSIGHT_EFFECTIVE_OPENCLAW_CONFIG="${AUTODEV_MEMORY_HINDSIGHT_EFFECTIVE_OPENCLAW_CONFIG:-$AUTODEV_STATE_DIR/openclaw-hindsight-local.json}"
AUTODEV_MEMORY_HINDSIGHT_RECOVERY_PROFILE="${AUTODEV_MEMORY_HINDSIGHT_RECOVERY_PROFILE:-autodev-local}"
AUTODEV_MEMORY_HINDSIGHT_RECOVERY_HOME="${AUTODEV_MEMORY_HINDSIGHT_RECOVERY_HOME:-$AUTODEV_MEMORY_BACKEND_DIR/hindsight-home}"
AUTODEV_MEMORY_HINDSIGHT_RECOVERY_UV_CACHE="${AUTODEV_MEMORY_HINDSIGHT_RECOVERY_UV_CACHE:-$AUTODEV_MEMORY_BACKEND_DIR/uv-cache}"
AUTODEV_MEMORY_HEALTH_STATUS_FILE="${AUTODEV_MEMORY_HEALTH_STATUS_FILE:-$AUTODEV_STATE_DIR/memory-health-status.txt}"
AUTODEV_MEMORY_HEALTH_REPORT_FILE="${AUTODEV_MEMORY_HEALTH_REPORT_FILE:-$AUTODEV_STATE_DIR/memory-health-latest.txt}"
AUTODEV_MEMORY_HEALTH_JSON_FILE="${AUTODEV_MEMORY_HEALTH_JSON_FILE:-$AUTODEV_STATE_DIR/memory-health.json}"
AUTODEV_MEMORY_HEALTH_REMEDIATION_FILE="${AUTODEV_MEMORY_HEALTH_REMEDIATION_FILE:-$AUTODEV_STATE_DIR/hindsight-config-remediation.json}"
AUTODEV_MEMORY_HEALTH_OPENCLAW_MODE="${AUTODEV_MEMORY_HEALTH_OPENCLAW_MODE:-auto}"
AUTODEV_MEMORY_HEALTH_OPENCLAW_PLUGIN="${AUTODEV_MEMORY_HEALTH_OPENCLAW_PLUGIN:-hindsight-openclaw}"
AUTODEV_MEMORY_HEALTH_SWARM_URL="${AUTODEV_MEMORY_HEALTH_SWARM_URL:-}"
AUTODEV_MEMORY_HEALTH_HTTP_TIMEOUT_SECONDS="${AUTODEV_MEMORY_HEALTH_HTTP_TIMEOUT_SECONDS:-5}"

export PROJECT_NAME AUTODEV_ID
export AUTODEV_MEMORY_BACKEND AUTODEV_MEMORY_HINDSIGHT_API_URL AUTODEV_MEMORY_HINDSIGHT_API_TOKEN
export AUTODEV_MEMORY_HINDSIGHT_BANK_ID AUTODEV_MEMORY_HINDSIGHT_BANK_MISSION AUTODEV_MEMORY_HINDSIGHT_OBSERVATIONS_MISSION
export AUTODEV_MEMORY_HINDSIGHT_STATE_FILE AUTODEV_MEMORY_HINDSIGHT_RECALL_BUDGET AUTODEV_MEMORY_HINDSIGHT_MAX_TOKENS
export AUTODEV_MEMORY_HINDSIGHT_TIMEOUT_SECONDS AUTODEV_MEMORY_HINDSIGHT_TAGS
export AUTODEV_MEMORY_HINDSIGHT_RECOVERY_OPENCLAW_CONFIG AUTODEV_MEMORY_HINDSIGHT_RECOVERY_PROFILE
export AUTODEV_MEMORY_HINDSIGHT_EFFECTIVE_OPENCLAW_CONFIG
export AUTODEV_MEMORY_HINDSIGHT_RECOVERY_HOME AUTODEV_MEMORY_HINDSIGHT_RECOVERY_UV_CACHE
export AUTODEV_LEARNED_SKILLS_DIR AUTODEV_LEARNED_SKILLS_SUMMARY_FILE AUTODEV_LEARNED_SKILLS_MAX
...
effective_hindsight_recovery_openclaw_config() {
  local effective_path

  effective_path="$(python3 "$AUTODEV_ROOT/automation/prepare-openclaw-hindsight-config.py" \
    --source "$AUTODEV_MEMORY_HINDSIGHT_RECOVERY_OPENCLAW_CONFIG" \
    --output "$AUTODEV_MEMORY_HINDSIGHT_EFFECTIVE_OPENCLAW_CONFIG" \
    --api-url "$AUTODEV_MEMORY_HINDSIGHT_API_URL" 2>/dev/null || true)"
  ...
  printf '%s\n' "$AUTODEV_MEMORY_HINDSIGHT_RECOVERY_OPENCLAW_CONFIG"
}

ensure_hindsight_api_available() {
  local openclaw_config_path

  case "$AUTODEV_MEMORY_BACKEND" in
    hindsight|hindsight-hybrid)
      ;;
    *)
      return 0
      ;;
  ...
  python3 "$AUTODEV_ROOT/automation/hindsight-recover.py" \
    --api-url "$AUTODEV_MEMORY_HINDSIGHT_API_URL" \
    --openclaw-config "$openclaw_config_path" \
    --profile "$AUTODEV_MEMORY_HINDSIGHT_RECOVERY_PROFILE" \
    --workspace-home "$AUTODEV_MEMORY_HINDSIGHT_RECOVERY_HOME" \
    --uv-cache-dir "$AUTODEV_MEMORY_HINDSIGHT_RECOVERY_UV_CACHE" \
...
refresh_skill_memory() {
  python3 "$AUTODEV_ROOT/automation/skill-memory-sync.py" \
    --autodev-root "$AUTODEV_ROOT" \
    --sessions-dir "$AUTODEV_SESSION_DIR" \
    --output-dir "$AUTODEV_LEARNED_SKILLS_DIR" \
    --summary-file "$AUTODEV_LEARNED_SKILLS_SUMMARY_FILE" \
    --project "$PROJECT_NAME" \
    --limit "$AUTODEV_LEARNED_SKILLS_MAX"
}
...
build_prompt_bundle() {
...

  {
    ...
    for file in \
      ...
      "$AUTODEV_LEARNED_SKILLS_SUMMARY_FILE" \
